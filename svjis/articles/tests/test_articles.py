...
class ArticleListTest(ArticleDataMixin, TestCase):
    def do_user_test(self, username, password, for_all, for_owners, for_board):
        # Login user
        ...
        # Main page still renders (its content is now fetched client-side from /api/v1/,
        # exercised separately by ArticleApiListTest/TopArticlesApiTest in test_api_homepage.py)
        response = self.client.get(reverse('main'))
        self.assertEqual(response.status_code, 200)

    def test_admin_user(self):
        self.do_user_test('jarda', self.u_jarda_password, 200, 200, 200)

    def test_board_user(self):
        self.do_user_test('jiri', self.u_jiri_password, 200, 200, 200)

    def test_owner_user(self):
        self.do_user_test('peter', self.u_peter_password, 200, 200, 404)

    def test_vendor_user(self):
        self.do_user_test('karel', self.u_karel_password, 200, 404, 404)

    def test_anonymous_user(self):
        self.do_user_test('anonymous', '', 200, 302, 302)
